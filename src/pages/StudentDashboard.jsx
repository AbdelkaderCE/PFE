...
/* ── Helpers ────────────────────────────────────────────────── */
const TYPE_BADGE = {
  Cours: 'bg-blue-50 text-brand border border-blue-200',
  TD:    'bg-amber-50 text-warning border border-amber-200',
  TP:    'bg-green-50 text-success border border-green-200',
};

const URGENCY_STYLES = {
  urgent: { dot: 'bg-danger',  label: 'text-danger',  badge: 'bg-red-50 text-danger border border-red-200' },
  soon:   { dot: 'bg-warning', label: 'text-warning', badge: 'bg-amber-50 text-warning border border-amber-200' },
  normal: { dot: 'bg-brand',   label: 'text-brand',   badge: 'bg-blue-50 text-brand border border-blue-200' },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          >
            <div className="shrink-0 w-9 h-9 rounded-lg bg-blue-50 flex items-center justify-center text-brand">
              {stat.icon}
            </div>
            <div>
            ...
            {DOCUMENTS.map((doc) => (
              <li key={doc.id} className="px-5 py-3.5 hover:bg-surface-200/50 transition-colors duration-100">
                <div className="flex items-center gap-3">
                  ...
                  {/* Download */}
                  <button
                    className="shrink-0 p-2 rounded-md text-brand hover:bg-blue-50 transition-colors duration-150"
